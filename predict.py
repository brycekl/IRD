...
def main():
    # init basic setting
    model_path = 'model/240530/poly/all_same_od_4-all_same_0.921'
    ...
    if task == 'landmark':
        result = {'name': [], 'll_mse': [], 'lr_mse': [], 'l_dis_gt': [], 'l_dis_pre': []}
    elif task == 'poly':
        result = {'name': [], 'pl_mse': [], 'pr_mse': [], 'l_dice': [], 'r_dice': [],
                  'p_dis_gt': [], 'p_dis_pre': []}
    else:
        result = {'name': [], 'll_mse': [], 'lr_mse': [], 'pl_mse': [], 'pr_mse': [], 'l_dice': [], 'r_dice': [],
    ...
    for img, target in val_data_loader:
        ...
        # if name != '394fzjyangzuying20240315-113841-6160__202403151315470057SMP':
        #     continue
        ...
        output = cv2.warpAffine(np.transpose(output, (1, 2, 0)), target['reverse_trans'][0],
        ...
        if task in ['landmark', 'all']:
            ...
            for i in range(2):
                pre = output[:, :, i]
                left_right = 'l' if i == 0 else 'r'
                y, x = np.where(pre == pre.max())
                landmark_pre[i+5] = [x[0], y[0]]
                point = landmark_gt[i + 5]  # label=i+8
                result[f'l{left_right}_mse'].append(spa_name * math.sqrt(math.pow(x[0] - point[0], 2) + math.pow(y[0] - point[1], 2)))
                # save heatmap result
            ...
            pre_target['l_mse'] = {5: result["ll_mse"][-1], 6: result["lr_mse"][-1]}
            pre_target['l_dis'] = {'l_dis_gt': dis_gt, 'l_dis_pre': dis_pre}

        if task in ['poly', 'all']:
            output_poly = np.argmax(output[:, :, -3:], axis=2)
            ...
            for i in range(2):
                ...
                spa_name * math.sqrt(math.pow(x[0] - point[0], 2) + math.pow(y[0] - point[1], 2))
                # calculate the error between landmark_Gt and the keypoint from poly
                result[f'p{left_right}_mse'].append(spa_name * math.sqrt(
                    math.pow(keypoint['gt'][i+5][0] - keypoint['pre'][i+5][0], 2) + math.pow(keypoint['gt'][i+5][1] - keypoint['pre'][i+5][1], 2)))
            ...
            pre_target['p_mse'] = {5: result["pl_mse"][-1], 6: result["pr_mse"][-1]}
